...
public class SetSpectatorCommand implements CommandExecutor {
    ...
    public SetSpectatorCommand( DevathonPlugin devathonPlugin ) {
        this.devathonPlugin = devathonPlugin;
    }

    @Override
    public boolean onCommand( CommandSender commandSender, Command command, String commandLabel, String[] args ) {
        if ( commandSender.hasPermission( "droidfactory.setspectator" ) ) {
            if ( commandSender instanceof Player ) {
                Player player = ( Player ) commandSender;
                this.devathonPlugin.getMapConfig().setSpectatorSpawn( new SpawnPoint( player.getLocation() ) );
                this.devathonPlugin.getMapConfig().saveConfig();
                player.sendMessage( this.devathonPlugin.getPrefix() + "§aSuccessfully set spectator spawn" );
            } else {
